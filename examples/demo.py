from teedeelee import CompleteTreeView, TopologyTreeView, MSCTreeView
...
def demo_complete_tree():
    ...
    # Create a CompleteTreeView
    tree = CompleteTreeView(newick)
    
    print(f"Number of taxa: {tree.ntaxa}")
    print(f"Original tree: {tree.to_newick_string()}")
    
    # Get distance matrix
    dist_matrix = tree.get_distance_matrix(0)
    if dist_matrix:
        print("\nPairwise distances:")
...
def demo_topology_tree():
    ...
    # Create a TopologyTreeView
    tree = TopologyTreeView(newick)
    
    print(f"Original tree: {tree.to_newick_string()}")
    
    # Demonstrate restriction to subset of taxa
    restricted = tree.restriction(["A", "C", "D"])
...
def demo_msc_tree():
    ...
    # Create MSC tree view
    msc = MSCTreeView(gene_trees, species_tree)
