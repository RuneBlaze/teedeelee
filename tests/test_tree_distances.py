...
def generate_random_newick(taxa_list, current_depth=0, max_depth=3):
    ...
    random.shuffle(taxa_list)
    split_points = sorted([0] + [random.randint(1, len(taxa_list)-1) 
                                for _ in range(n_children-1)] + [len(taxa_list)])
    groups = [taxa_list[split_points[i]:split_points[i+1]] 
...
def test_restriction_edge_cases():
    ...
    assert dist_matrix.get_by_name("A", "B") == 2.0, \
    ...
    dist_matrix = restricted.get_distance_matrix(0)
    assert dist_matrix.get_by_name("A", "B") == 2.0, "Distance between close pair incorrect"
