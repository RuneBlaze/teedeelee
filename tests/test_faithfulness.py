...
def load_test_data(base_dir: str = "/Users/lbq/Downloads/S101", num_datasets: int = 5) -> Tuple[List[str], List[str]]:
    """
...
def check_tree_faithfulness(species_tree_file: str, gene_trees_file: str, subset: List[str]) -> bool:
    """
...
def test_restriction_faithfulness():
    ...
    for gene_trees_file, species_tree_file in zip(gene_trees_files, species_tree_files):
        ...
        for subset_size in [3, len(all_taxa)//2, len(all_taxa)]:
            ...
            assert check_tree_faithfulness(species_tree_file, gene_trees_file, subset), \
...
def test_family_msc_faithfulness():
    ...
    assert len(family) == len(gene_trees_files), \
    ...
    for i, msc_set in enumerate(family):
        ...
        for subset_size in [3, len(all_taxa)//2, len(all_taxa)]:
            ...
            for tree in restricted_set:
                ...
                assert restricted_taxa == set(subset), \
...
def test_edge_cases():
    ...
    assert check_tree_faithfulness(species_tree_files[0], gene_trees_files[0], min_subset), \
        "Faithfulness not preserved for minimum subset"
    
    # Test subset preserving internal node
    internal_subset = all_taxa[:3]
    assert check_tree_faithfulness(species_tree_files[0], gene_trees_files[0], internal_subset), \
        "Faithfulness not preserved when keeping internal node"
    
    # Test with all taxa
    assert check_tree_faithfulness(species_tree_files[0], gene_trees_files[0], all_taxa), \
...
def normalize_tree_str(tree_str: str) -> str:
    ...
    ts_tree.order("num_descendants_then_label") # This helps standardize the internal structure
